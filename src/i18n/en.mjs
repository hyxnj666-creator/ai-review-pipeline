export default {
  ...
  builtinFallback: 'Using built-in free model (SiliconFlow). Test generation is skipped — configure your own API Key to unlock it.',
  ...
  score: (s, r, y, g, b = 0) => `Score: ${s}/100 | 🔴${r} 🟡${y} 🟢${g}${b ? ` 🔵${b}` : ''}`,
  ruleEngineFound: (n) => `Rule engine flagged ${n} deterministic issue(s)`,
  passed: (s, t) => `Quality passed (${s} >= ${t}). Review passed!`,
  gateBlocked: (reasons) => `Merge gate blocked: ${reasons}`,
  finalCounts: (r, y, g, b) => `Issues: 🔴${r} blocker · 🟡${y} major · 🟢${g} minor · 🔵${b} info`,
  finalGate: (th, mm) => `Gate: score >= ${th}, 🟡 <= ${mm}, any 🔴 blocks`,
...
Gate options (both review and fix):
  --threshold <n>     Score threshold (default: 85)
  --max-major <n>     Max allowed 🟡 majors (default: 3). Any 🔴 blocker always blocks.
...
Severity & scoring (commercial gate):
  🔴 Blocker -25 each   — any single 🔴 blocks the merge
  🟡 Major   -5  each   — more than --max-major (default 3) blocks the merge
  🟢 Minor   -1  each   — affects score only
  🔵 Info    0 each, density -1 per 🔵 over 5, cap -3

Exit codes:
  0   Gate passed (no 🔴, 🟡 <= max-major, score >= threshold)
  1   Gate blocked (any 🔴, too many 🟡, score below threshold, or --fix exhausted)
